...
const getHeaderLayout = async () => {
  const {
    HeaderLayout
  } = await import(/* webpackChunkName: "HeaderLayout" */ "layouts/HeaderLayout");
  return HeaderLayout;
};

// Helper to directly load components and layouts
const getComponent = <T>(component: T) => async () =>
